...
import { getAllRoutes, registerDriver } from '../api/admin';
...
const Registerdriver = () => {
	...
	const handleSubmit = async (e: any) => {
		e.preventDefault();
		dispatch(showLoading());
		const registerDriverResponce: any = await registerDriver({ ...driverData, photo, validId });
		dispatch(hideLoading());
		
		if (registerDriverResponce.status == 201) {
			toast.success(registerDriverResponce.data.message);
			navigate('/admin/alldrivers');
		} else if (registerDriverResponce.status == 400) {
			toast.error(registerDriverResponce.data.message);
		}
	};
	...
	return (
		<>
			...
			<div className='flex bg-[#e9ebf0] items-center justify-center gap-16 pt-8 pl-6 pr-10 pb-14 text-sm '>
				<div className='flex flex-col w-[676px] items-center justify-center max-[760px]:w-[560px] max-[610px]:w-[350px] mt-[7rem] ml-[15rem]'>
					<div className='flex items-center justify-between w-full'>
						<h1 className='text-3xl'>Register a Driver</h1>
